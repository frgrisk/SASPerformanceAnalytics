%macro return_accumulate_test2(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\return_accumulate_test2_submit.sas";
%end;
...
%return_accumulate(prices,method=LOG,toFreq=MONTH,updateInPlace=FALSE)
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST RETURN_ACCUMULATE_TEST2;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST RETURN_ACCUMULATE_TEST2;
	%let pass=FALSE;
