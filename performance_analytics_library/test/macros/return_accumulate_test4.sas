%macro return_accumulate_test4(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\return_accumulate_test4_submit.sas";
%end;
...
put "returns = apply.quarterly(returns,FUN=Return.cumulative,geometric=FALSE)";
...
%return_accumulate(prices,method=LOG,toFreq=QUARTER,updateInPlace=FALSE)
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST RETURN_ACCUMULATE_TEST4;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST RETURN_ACCUMULATE_TEST4;
	%let pass=FALSE;
