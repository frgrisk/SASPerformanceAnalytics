%macro Market_Timing_test2(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\Market_Timing_test2_submit.sas";
%end;
...
put "returns = MarketTiming(returns[,1:4], returns[,5], Rf = 0.01/252, method = 'TM')";
put "endsubmit;";
run;
...
%Market_Timing(prices,BM=SPY,Rf= 0.01/252,option= TM)
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST MARKET_TIMING_TEST2;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST MARKET_TIMING_TEST2;
	%let pass=FALSE;
