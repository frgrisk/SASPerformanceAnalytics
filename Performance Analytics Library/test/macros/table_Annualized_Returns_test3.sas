%macro table_Annualized_Returns_test3(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\table_Annualized_Returns_test3_submit.sas";
%end;
...
put "returns = table.AnnualizedReturns(returns, Rf= 0.01/4, scale= 4, geometric=FALSE, digits=6)";
put "endsubmit;";
run;
...
%table_Annualized_Returns(prices, Rf= 0.01/4, scale= 4, method=LOG, digits=6)
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST table_Annualized_Returns_TEST3;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST table_Annualized_Returns_TEST3;
	%let pass=FALSE;
