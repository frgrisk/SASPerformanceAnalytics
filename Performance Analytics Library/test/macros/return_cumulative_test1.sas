%macro return_cumulative_test1(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\return_cumulative_test1_submit.sas";
%end;
...
put "returns = Return.cumulative(returns, geometric=TRUE)";
...
%return_cumulative(prices,method=GEOMETRIC)
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST RETURN_CUMULATIVE_TEST1;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST RETURN_CUMULATIVE_TEST1;
	%let pass=FALSE;
