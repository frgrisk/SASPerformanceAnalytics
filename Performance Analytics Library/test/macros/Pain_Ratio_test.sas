%macro Pain_Ratio_test(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\Pain_Ratio_test_submit.sas";
%end;
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST PAIN_RATIO_TEST;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST PAIN_RATIO_TEST;
	%let pass=FALSE;
