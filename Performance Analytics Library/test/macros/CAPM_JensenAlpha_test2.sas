%macro CAPM_JensenAlpha_test2(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\CAPM_JensenAlpha_test2_submit.sas";
%end;
...
put "returns = CAPM.jensenAlpha(returns[, 1:4], returns[,5], Rf= 0.01/12)";
put "endsubmit;";
run;
...
%Edit_returns(prices)
%CAPM_JensenAlpha(prices, BM= SPY, Rf= 0.01/12, scale= 12, outData= Jensen_Alpha)
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST CAPM_JensenAlpha_test2;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST CAPM_JensenAlpha_test2;
	%let pass=FALSE;
