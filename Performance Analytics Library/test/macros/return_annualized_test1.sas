%macro return_annualized_test1(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\return_annualized_test1_submit.sas";
%end;
...
put "returns = Return.annualized(returns, scale= 252, geometric=TRUE)";
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST RETURN_ANNUALIZED_TEST1;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST RETURN_ANNUALIZED_TEST1;
	%let pass=FALSE;
