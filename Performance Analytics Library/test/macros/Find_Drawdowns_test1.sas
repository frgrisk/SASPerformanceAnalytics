%macro Find_Drawdowns_test1(keep=FALSE);
%global pass notes;
...
%end;
%else %do;
	filename x "&dir\Find_Drawdowns_test1_submit.sas";
%end;
...
put "drawdowns=findDrawdowns(returns, geometric=TRUE)";
...
%Find_Drawdowns(prices,asset=IBM,method= DISCRETE,SortDrawdown= FALSE);
...
%if &n = 0 %then %do;
	%put NOTE: NO ERROR IN TEST FIND_DRAWDOWNS_TEST1;
	%let pass=TRUE;
	%let notes=Passed;
%end;
%else %do;
	%put ERROR: PROBLEM IN TEST FIND_DRAWDOWNS_TEST1;
	%let pass=FALSE;
