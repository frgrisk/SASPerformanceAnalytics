%macro ActivePremium_test1(keep=FALSE);
%global pass notes;
...
put "returns = ActivePremium(returns[, 1:4], returns[,5])";
put "endsubmit;";
run;
...
%ActivePremium(prices, BM= SPY, scale= 252, outActivePremium= active_premium)
